...
class TuiSpec extends WordSpec with Matchers{
  ...
  "A Go Tui" should {
    ...
    "create and empty Sudoku on input 'n'" in {
      ...
      controller.asGame should be(new Grid(9), (Player("Player 1", Cell(CellStatus.BLACK)), Player("Player 2", Cell(CellStatus.WHITE))))
    }
    ...
    "set Grid size on 3 with player names: Bob and Elly with input 'n 3 Bob Elly'" in {
      ...
      controller.asGame should be(new Grid(3), (Player("Bob", Cell(CellStatus.BLACK)), Player("Elly", Cell(CellStatus.WHITE))))
    }
  }
